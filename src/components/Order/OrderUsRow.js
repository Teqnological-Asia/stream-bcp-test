...
const OrderUsRow = ({order,loadUs,tab}) => {
    ...
    const renderStatusLink = (order) => {
        return (
            <Link className="c-u" to={`/account/order/${order.orderID}/detail`}>{statuses[order.status]}</Link>
        );
    }
    ...
    return (
        <tr>
            <td className="c-action" >
                {order.isCancelable && (
                    <Link className="icon-cancel-circled" to={`/account/order/${order.orderID}/cancel?tab=${tab}`} >
                        <span>取り消す</span>
                    </Link>
                )}
            </td>
            ...
            <td className="c-l" data-name="（出来済）">{order.filledQuantity != null ? formatCurrency(order.filledQuantity, 0) : 0}</td>
